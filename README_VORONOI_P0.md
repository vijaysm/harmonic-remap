# Lowest-Order Mimetic Voronoi-to-Voronoi Edge-Flux Remap
...
docs/figures/voronoi_p0/                    convergence + projection + divergence figures
```

## Known limitations / future work

- **Planar only.** Spherical extension via local gnomonic projection
  (matching the C++ codebase's `SphericalGnomonic` mode) has not been
  ported. Convergence on MPAS-style spherical Voronoi meshes is the
  natural next step.
- **Lowest order.** Higher-order edge-moment transfer (`p ≥ 1`)
  available in the C++ `PlanarMomentInterpolator` is out of scope here;
  this script targets the simplest `p=0` case for clarity.
