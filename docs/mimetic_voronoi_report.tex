...
\begin{table}[t]
  ...
  \(\int_{\partial O}\uh\cdot\n ds = d_s|O|\) on each overlap and
  aggregate \(\int_\Omega\nabla\cdot(x,y)dA=2\) &
  \(4.44\times10^{-16}\) \\
  Voronoi n-gon overlay &
  Nine source cells, 14 target cells, 36 overlaps, 23 n-gon to n-gon overlaps &
  Same overlap invariant plus target-cell area coverage &
  \(4.44\times10^{-16}\) \\
  \bottomrule
  \end{tabular}
...
The results show that the conservative property is controlled by two local
ingredients: exact reconstruction of the polynomial moments used by the
level-2 basis and exact reduction over each clipped overlap boundary.  For the
...
flux is \(2|O|\), and the residuals in Table~\ref{tab:verification-summary}
measure only floating-point and clipping-roundoff effects.
